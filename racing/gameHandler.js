...
startScreen.addEventListener("click", () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    console.log(carIcon);
    enemyCar.innerHTML = carIcon;
    enemyCar.setAttribute("class", "enemyCar");
    enemyCar.y = (i + 1) * 350 * -1;
    enemyCar.style.top = enemyCar.y + "px";
    enemyCar.style.fill = randomColor();
...
window.onload = async () => {
  const imageResponse = await fetch(`./assets/images/car.svg`);
  carIcon = await imageResponse.text();
};
